...
let changeURL = address => history.pushState(null, null, `/wallet-explorer/?q=${address}`);

let numberWithSpaces = coins => {
	// the function adds spaces to large numbers
	if (coins == 0) return;
	if (String(coins).length == 1) return coins;
...
async function getAddressInfo(address) {
	...
	document.getElementById("wallet-balance").innerHTML = `${ton} TON`;
	document.getElementById("main-wallet-block").style.display = "block";
...
function transactionBlock(base) {
	...
	function createTransactionsList(data) {
		...
		data.forEach((item, index) => {
			if (index == 0) {
				...
				if (data[1] == "From") {
					balanceDiv.innerHTML = `+${item} TON`;
					balanceDiv.style.color = "lime";
				}
				else {
					balanceDiv.innerHTML = `-${item} TON`;
	...
	for (i = 0; i < base.length; i++) {
		if (base[i]['out_msgs'].length == 0) {
			...
			let tfee = `${getTonPrice(base[i]['fee'])} TON`;
